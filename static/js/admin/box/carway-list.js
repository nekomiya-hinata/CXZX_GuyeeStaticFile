...
var DemoList = function () {
    ...
    var initPage = function (that) {
        ...
        $('.btn_add').on('click', function () {
        	var boxid=$("#boxid").val();
			window.open(ctx.concat("/admin/guyeecarpassageway/wayadd?boxid="+boxid));
        });
        ...
        this.datatable = $('#table').DataTable({
            ...
            "ajax": {
                "url": ctx + "/admin/guyeecarpassageway/quickSearch",
                "type": "GET",
    ...
    return {
        ...
        view: function (id) {
        	window.open(ctx.concat("/admin/guyeecarpassageway/info?id=",id));
        },
        edit: function (id) {
        	window.open(ctx + '/admin/guyeecarpassageway/wayedit?id=' + id);
        },
        del: function (id) {
			form_helpers.confirm('确定删除?（删除时会将该通道下的白名单一起删除）', function(result) {
				if (result.value == true) {
					...
					$.ajax({
						type : "POST",
						url : ctx + '/admin/guyeecarpassageway/delete',
						data : {
							'id' : id,
						},
