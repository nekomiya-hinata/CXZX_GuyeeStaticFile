    ...
    var initPage = function (that) {
    	...
    	waterControl.prototype.initialize = function(map){
     	...
     	/*获取相似车辆列表*/
     	 ...
     	 $('#form').ajaxForm({
     		url :ctx + "/gov/caraccesslist/searchSimilarityCar",
			dataType : 'json',
			type : 'POST',
		...
		//人员轨迹查询
		this.searchTrack = function searchTrack(plate){
			$.ajax({
    			type : "GET",
    			url :ctx + "/gov/caraccesslist/searchTrack?plate="+plate+"&sdate="+$("#sdate").val()+"&edate="+$("#edate").val(),
    			success : function(data) {
    				var points = [];
    				clear();
    				if(data!=null){
